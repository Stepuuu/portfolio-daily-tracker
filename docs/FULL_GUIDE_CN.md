> v3.1：交易录入请优先使用 [账本指南](LEDGER.md)。AI 生成方案后由用户在页面确认；截图识别不会自动入账。下文保留旧版 CLI、定时任务和通知配置供尚未迁移的用户参考。启用账本前请停用会自动修改旧持仓文件的定时任务。无需 Docker，可在已有容器实例中直接运行。
...
OpenClaw 是一个可以接收飞书/Telegram 消息、用 AI 自动管理持仓的 Agent 框架。配置后，你可以直接给 Bot 发消息「今天卖了500股茅台」，它可以生成待确认的交易方案，核对后在账本页面确认；日报生成与入账分开处理。
...
→ Bot 生成方案 → 页面确认 → 按已配置的授权单独生成快照和推送日报
或
→ 你回复："未变化"
→ Bot 直接生成快照 + 推送日报
```
